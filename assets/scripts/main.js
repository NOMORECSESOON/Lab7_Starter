...
async function getRecipes() {
  ...
  let recipes_in_local_storage = JSON.parse(localStorage.getItem('recipes'));
  if (localStorage.getItem('recipes') !== null) {
    return recipes_in_local_storage;
  }
  ...
  let promise = new Promise(async(resolve, reject) => {
    for (let i = 0; i < RECIPE_URLS.length; i++) {
      try {
        ...
        store_recipe.push(newrecipe);
      } catch (error) {
        console.error(error);
        reject(error);
      }
    }
    saveRecipesToStorage(store_recipe);
    resolve(store_recipe);
  });
  return promise;
